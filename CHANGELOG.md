...
- Fix critical regression (issue #131) : no notifications_manager email
  worked since #104 merged, silently (caught exception, WARNING log only,
  never surfaced). `smtp.send_message`'s real schema has no `target`
  field - #104 verified the wrong source (legacy `notify/legacy.py`
  instead of the actual entity-based smtp integration).
- Add `_resolve_or_create_smtp_recipient()`: resolves the `notify.*`
  entity for a given recipient email via the smtp integration's
  "recipient subentry" mechanism (HA 2026.7+), creating one on demand
  via `hass.config_entries.async_add_subentry()` if missing. Removes the
  hardcoded `entity_id: notify.notify_smtp` and the invalid `target`
  field entirely.
- Verified with real sends in DEV to two different recipients
  (2026-09-10).

## 0.3.3

- Add systematic date suffix on email subject (` | dd/mm/YYYY`), skipped when
  the caller-supplied title already contains a `jj/mm` pattern (issue #19).
- Add standardized footer on email body: signature line + emission
  timestamp + unsubscribe notice, generated by the service, never by the
  calling package (issue #19).
- Push notifications unaffected (title/message unchanged).
- Bump component version to 0.3.3.
